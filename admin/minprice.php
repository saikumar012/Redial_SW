...
if(isset($_GET['min_delete']))
{
   $id1=$_GET['min_delete'];
   $sql=$database->query("delete from minimum where sno='$id1'");
...
<div class="r_menu2"><a href="minprice.html" class="active">Min Price Range</a><a href="maxprice.html">Max Price Range</a></div>

<div style="width:100%; float:left;">

<div class="main_hed2"><img src="../images/p_sicon.png" align="absmiddle" /> &nbsp; Min Price Range</div>
<?php
$query = "select * from minimum order by min ASC";
...
$query_comp = $database->query("select * from minimum where min = '$price' and Type = '$price_type' ");
if(mysql_num_rows($query_comp)>0)
{
echo "<p style='color:#FF0000'>price range already exist</p>";
} else { 
$sql=$database->query("insert into minimum values('','$price','$price_type')");
header("location:minprice.php");
exit;
}
}
...
<div id="form">
<ol>
<p><li><label>Min Price</label><input class="input" name="min" value=""/></li></p>
...
<td><?=$row['min']?></td>
<td><?=@$type?></td>
<td><a onclick="deletee('<?=$row['sno']?>')"><img src="../images/close.png" /></a></td>
</tr>
