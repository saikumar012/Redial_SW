...
if(!$session->logged_in){
    ...
    $type = 1; //residential
    $project_list=$database->getproject_list($type);
...
<div class="left_body">
<?php while($row=mysql_fetch_array($project_list)) {  ?>
...
<div class="div propertytype"><?=$database->getPropertyTypeById($row['property_type']); ?></div>

<div class="div area"><?php if($row['want']==1){echo "Sell";}else{ echo "Rent";} ?></div>
</div>
<div style="float:left;">
<div class="div city"><?=$database->getCityById($row['city']); ?></div>
<div class="div whareincity"><?=$database->getLocationById($row['location']); ?></div>
</div>
<div style="float:left;">
...
<a href="projects_addpost2.php?projectid=<?php echo $row['project_id']; ?>"><img src="images/pr_view.png" style=" position:absolute; bottom:-25px; right:20px;" /></a>
